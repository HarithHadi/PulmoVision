...
# ── Model (same class as tb_train.py) ─────────────────────────────────────────
class RADDINOClassifier(nn.Module):
    ...
    def _register_hooks(self):
        ...
        def forward_hook(module, input, output):
            # output is a tuple — first element is hidden states [B, seq_len, D]
            out = output[0] if isinstance(output, tuple) else output
            self._activations = out.detach()

        def backward_hook(module, grad_input, grad_output):
            # grad_output is a tuple — first element is gradient [B, seq_len, D]
    ...
    def get_gradcam(self, pixel_values, target_class=1):
        # Enable gradients for entire backbone
        for param in self.backbone.parameters():
        ...
        # Forward pass — must NOT be inside torch.no_grad()
        ...
        if self._gradients is None or self._activations is None:
            raise RuntimeError("GradCAM hooks did not fire — check hook registration.")

        print(f"Grads shape: {self._gradients.shape}, Acts shape: {self._activations.shape}")
        print(f"Grads norm: {self._gradients.norm():.6f}")

        # Both are [B, seq_len, D] — remove CLS token at index 0
        grads = self._gradients[0, 1:, :]   # [N_patches, D]
        acts  = self._activations[0, 1:, :] # [N_patches, D]
        ...
        print(f"CAM before norm — min: {cam.min():.6f}, max: {cam.max():.6f}")
...
tb_model.load_state_dict(torch.load(MODEL_SAVE, map_location=DEVICE, weights_only=False))
tb_model.eval()
